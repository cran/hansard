...
#' @param extra_args Additional parameters to pass to API. See the \href{http://explore.data.parliament.uk/}{API documentation} or the package vignette for more details. Defaults to \code{NULL}.
#' @param tidy Logical parameter. If \code{TRUE}, fixes the variable names in the tibble to remove special characters and superfluous text, and converts the variable names to a consistent style. Defaults to \code{TRUE}.
...
all_answered_questions <- function(mp_id = NULL, tabling_mp_id = NULL, house = NULL, answering_body=NULL, start_date = "1900-01-01", end_date = Sys.Date(), extra_args = NULL, tidy = TRUE, tidy_style = "snake_case", verbose = FALSE) {
    ...
    if (length(mp_id) > 1 || length(tabling_mp_id) > 1  || length(answering_body) > 1) {
      ...
      if (is.null(house) == TRUE) {# House query
    ...
    if (is.null(answering_body) == TRUE || is.na(answering_body)==TRUE) {# Department query
