...
test_that("edm functions return expected format", {
    ...
    xedmids <- hansard_early_day_motions(edm_id = 1073, session = "2015/16", verbose=TRUE)
    expect_length(xedmids, 11)
    expect_type(xedmids, "list")
    expect_true(tibble::is_tibble(xedmids))
    expect_equal(nrow(xedmids), 1)

    xedmid_full <- hansard_mp_edms(mp_id = 3967, primary_sponsor = TRUE, sponsor = FALSE, signatory = FALSE, full_data = TRUE, end_date = "2015-02-11", start_date = "2015-02-10", verbose=TRUE)
