...
test_that("all_answered_questions return expected format", {
    ...
    anameid <- hansard_all_answered_questions(house="lords", answering_body=60, start_date ='2017-03-01')
    expect_length(anameid, 30)
    expect_type(anameid, "list")
    expect_true(tibble::is_tibble(anameid))

    bidname <- hansard_all_answered_questions(house=2, answering_body="Education", start_date ='2017-03-01')
