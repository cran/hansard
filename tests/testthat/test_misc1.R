...
test_that("papers laid, publications, sessions, tv functions return expected format", {
    ...
    publjan <- hansard_publication_logs(start_date = "2016-01-01", end_date = "2016-01-10", verbose=TRUE)
    expect_length(publjan, 5)
    expect_type(publjan, "list")
    expect_true(tibble::is_tibble(publjan))
    expect_equal(nrow(publjan), 2)

    publcommons <- hansard_publication_logs(house="commons", start_date = "2016-01-01", end_date = "2016-01-15", tidy=FALSE, verbose=TRUE)
    ...
    # TV Programmes
    tvcoms <- hansard_tv_programmes("commons", start_date = "2016-12-19", end_date = "2017-01-01", verbose=TRUE)
